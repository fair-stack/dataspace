package cn.cnic.dataspace.api.datax.admin.core.trigger;
...
import cn.cnic.dataspace.api.datax.admin.util.JSONUtils;
...
public class JobTrigger {
    ...
    public static void trigger(int jobId, TriggerTypeEnum triggerType, int failRetryCount, String executorShardingParam, String executorParam) {
        JobInfo jobInfo = JobAdminConfig.getAdminConfig().getJobInfoMapper().selectById(jobId);
        if (jobInfo == null) {
            logger.warn(">>>>>>>>>>>> trigger fail, jobId invalid，jobId={}", jobId);
            return;
        }
        if (GlueTypeEnum.DATAX.getDesc().equals(jobInfo.getGlueType())) {
            // Decrypting account secrets
    ...
    private static void processTrigger(JobGroup group, JobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total) {
        ...
        jobLog.setSpaceId(jobInfo.getSpaceId());
        jobLog.setDataMappingId(jobInfo.getDataMappingId());
        ...
        triggerParam.setDataMappingId(jobInfo.getDataMappingId());
        // increment parameter
