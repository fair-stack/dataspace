...
public class ConnectionHandler {
    ...
    public void registerCommands() {
        ...
        // Passive mode is enabled by default, it is recommended to access an external network proxy
        // Passive Mode
        con.registerCommand("PASV", "PASV", this::pasv);
        // Active mode must be turned off, passive mode can support external network proxy mapping
        // Active Mode
    ...
    private void pasv() throws IOException {
        ...
        int paPort = random.nextInt(server.getPASSIVE_MIN_PORT()) + server.getPASSIVE_MAX_PORT();
        Socket socket = con.getSocket();
        InetAddress inetAddress = socket.getInetAddress();
        Utils.portCheck(paPort, inetAddress.getHostAddress());
        passiveServer = Utils.createServer(paPort, 5, server.getAddress(), server.getSSLContext(), secureData);
        passive = true;
        Utils.serMap.put(paPort, passiveServer);
        String host = passiveServer.getInetAddress().getHostAddress();
        // Log. info ("IP address obtained: {}"+host);
        // Log. info ("obtained local machine ip: {}"+InetAddress. getLocalHost(). getHostAddress());
        ...
        if (host.equals("0.0.0.0") || host.equals("127.0.0.1") || host.equals("localhost")) {
            ...
            host = Utils.getIpAddr(server.getIP_HOST());
            if (null == host) {
                host = InetAddress.getLocalHost().getHostAddress();
            }
        }
    ...
    private void auth(String mechanism) throws IOException {
        ...
        if (mechanism.equals("TLS") || mechanism.equals("TLS-C") || mechanism.equals("SSL") || mechanism.equals("TLS-P")) {
